...
type WebSocketClient = {
  ...
  terminate(): void;
  on(event: "open", listener: () => void): WebSocketClient;
  on(event: "message", listener: (data: WebSocketData) => void): WebSocketClient;
  on(event: "error", listener: (error: Error) => void): WebSocketClient;
  on(event: "close", listener: () => void): WebSocketClient;
  on(
...
export async function codexWebSocketResponse(input: {
  ...
  if (isBunRuntime) {
    ...
    return codexBunWebSocketResponse(input);
  }
  ...
  const ws = new WebSocket(wsUrl, {
    headers,
    agent,
    handshakeTimeout: CODEX_RESPONSES_WEBSOCKET_HANDSHAKE_MS,
...
async function codexBunWebSocketResponse(input: {
  httpUrl: string;
  headers: HeadersInit;
  payload: Record<string, unknown>;
  timeoutMs?: number;
...
function webSocketDataToText(data: WebSocketData) {
