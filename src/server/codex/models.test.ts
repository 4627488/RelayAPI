...
describe("Codex models manifest", () => {
  test("provides complete metadata for discovered Grok models", () => {
    ...
    expect(grok).toMatchObject({ slug: "grok-new", display_name: "Grok New", context_window: 500000, max_context_window: 500000, visibility: "list", supported_in_api: true, prefer_websockets: false, supports_search_tool: false });
    expect(grok?.supported_reasoning_levels).toEqual(expect.arrayContaining([expect.objectContaining({ effort: "medium" })]));
    expect(grok).not.toHaveProperty("upgrade");
  });

  test("keeps the existing RelayAPI metadata path for Codex models", () => {
    expect(codexManifestEntry({ id: "gpt-test", object: "model" }, 0)).toMatchObject({ slug: "gpt-test", priority: 1 });
  });
});
