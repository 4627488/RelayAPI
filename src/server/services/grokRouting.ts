...
export function selectGrokChannel(input: { model: string; apiKey: RelayApiKeyContext; excludedCredentialIds?: Set<string> }) {
  const now = Date.now(); const eligible = input.apiKey.tenantId ? new Set(eligibleCredentialIdsForTenant(input.apiKey.tenantId)) : null;
  ...
  markChannelUsed(selected.channel.id); updateGrokCredential(selected.credential.id, { lastUsedAt: new Date().toISOString() });
