...
export async function getGrokQuota(id: string): Promise<GrokQuotaReport> {
  ...
  if (credential.authType !== "oauth")
    throw new HttpError(
      400,
      "grok_quota_oauth_required",
      "Upstream quota is only available for Grok OAuth subscriptions",
    );
  ...
  const fetchBilling = (path: string) =>
    proxiedFetch(
      `${base}${path}`,
      {
  ...
  let responses = await Promise.allSettled([
    fetchBilling("/billing?format=credits"),
    fetchBilling("/billing"),
  ]);
  if (
    responses.some(
      (result) =>
    ...
    for (const result of responses)
      if (result.status === "fulfilled")
        await result.value.body?.cancel().catch(() => undefined);
  ...
  if (!successful.length) {
    ...
    throw new HttpError(
      ...
      `Grok billing request failed (${statuses})`,
    );
  }
  ...
  if (planType && planType !== credential.planType)
    updateGrokCredential(id, { planType });
  if (weekly) {
    ...
    if (weekly.resetsAt) {
      synchronizeSubscriptionQuotaWindows(
        listTenantSubscriptions()
          .filter((subscription) => subscription.credentialId === credential.id)
...
function weeklyWindow(config: BillingConfig): GrokQuotaWindow | null {
  ...
  const weekly =
    ...
    period.type.toLowerCase().includes("weekly");
  if (usedPercent === null && !weekly && !config.productUsage?.length)
    return null;
...
function monthlyWindow(config: BillingConfig): GrokQuotaWindow | null {
  ...
  if (limit === null && used === null && !text(config.billingPeriodEnd))
    return null;
  const usedPercent =
...
function quotaWindow(
  ...
  return {
    ...
    remainingPercent: normalized === null ? null : 100 - normalized,
    resetsAt,
  };
}
...
async function readBillingResponse(
  ...
  if (result.status === "rejected")
    return { ok: false, status: null, config: null };
  const response = result.value;
  if (!response.ok) {
    await response.body?.cancel().catch(() => undefined);
    return { ok: false, status: response.status, config: null };
  }
  try {
    return {
      ok: true,
      status: response.status,
      config: parseGrokBillingPayload(await response.json()),
    };
  } catch {
    return { ok: false, status: response.status, config: null };
  }
