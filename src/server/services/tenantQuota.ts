...
export function admitTenantRequest(input: { tenantId: string; tenantUserId: string; credentialId: string; requestId: string; model: string; now?: Date }): TenantQuotaAdmission {
  ...
  for (const subscription of candidates) {
    ...
    if (!limits) {
      return { requestId: input.requestId, tenantId: input.tenantId, subscriptionId: subscription.id, units: subscription.units, unitsPerCredential: subscription.unitsPerCredential, price, state: null };
    }
    const reserve = max(1n, min(10_000_000n, min(limits["5h"], limits["7d"]) / 100n));
    const now = input.now || new Date();
    const resetTimes = subscriptionResetTimes(providerCredential.provider, input.credentialId, subscription.id, now);
    try {
...
function subscriptionResetTimes(provider: "codex" | "grok", credentialId: string, subscriptionId: string, now = new Date()) {
  if (provider === "codex") {
    ...
    if (!five || !seven) throw new HttpError(503, "credential_quota_not_cached", "Credential quota must be refreshed before its subscriptions can be used");
    return { "5h": five, "7d": seven };
  }
  const existing = getSubscriptionQuotaState(subscriptionId).windows;
  const futureOr = (value: string | undefined, durationMs: number) => value && value > now.toISOString() ? value : new Date(now.getTime() + durationMs).toISOString();
  return { "5h": futureOr(existing["5h"]?.resetsAt, 5 * 60 * 60 * 1000), "7d": futureOr(existing["7d"]?.resetsAt, 7 * 24 * 60 * 60 * 1000) };
}
