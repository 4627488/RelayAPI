...
export function listSubscriptions(tenantId?: string, tenantUserId?: string) {
  const credentials = new Map<
    string,
    ReturnType<typeof getProviderCredential>
  >();
  const observedResets = new Map<
    string,
    ReturnType<typeof getProviderQuotaResetTimes>
  >();
  return listTenantSubscriptions(tenantId)
    .filter(
      (subscription) =>
        !tenantUserId || subscription.tenantUserId === tenantUserId,
    )
    .map((subscription) => {
      const currentLimits = subscriptionQuotaLimits(subscription);
      if (!credentials.has(subscription.credentialId)) {
        credentials.set(
          subscription.credentialId,
          getProviderCredential(subscription.credentialId),
        );
      }
      if (!observedResets.has(subscription.credentialId)) {
        observedResets.set(
          subscription.credentialId,
          getProviderQuotaResetTimes(subscription.credentialId),
        );
      }
      const credential = credentials.get(subscription.credentialId);
      const credentialResets = observedResets.get(subscription.credentialId)!;
      ...
      return {
        ...
        quota: Object.fromEntries(
          Object.entries(
            getSubscriptionQuotaState(subscription.id).windows,
          ).map(([kind, window]) => [
            kind,
            {
              limitNanoUsd: String(
                currentLimits?.[kind as "5h" | "7d"] ?? window.limitNanoUsd,
              ),
              ...
              resetSource:
                credential &&
                (providerCapability(credential.provider).quotaResetStrategy ===
                  "codex-cache" ||
                  credentialResets[kind as "5h" | "7d"] === window.resetsAt)
                  ? "upstream"
                  : "local",
            },
          ]),
...
export function getSubscriptionAllocationOverview() {
  ...
  const pools = listProviderCredentials().map((credential) => {
    ...
    const capacityUnits = providerCapacityUnits(
      credential.provider,
      credential.planType,
    );
    const normalizedAllocations = allocations.map((item) => ({
      ...item,
      allocatedPoolUnits:
        (item.units / item.unitsPerCredential) * capacityUnits,
    }));
    ...
    return {
      ...
      activeAllocationCount: normalizedAllocations.filter(
        (item) => item.lifecycle === "active",
      ).length,
      automaticQuotaSupported: providerSupportsAutomaticQuota(
        credential.provider,
        credential.provider === "grok" ? credential.authType : undefined,
      ),
      quotaResetStrategy: providerCapability(credential.provider)
        .quotaResetStrategy,
      quotaEstimates: Object.fromEntries(
        (["5h", "7d"] as const).map((kind) => [
          kind,
          {
            automaticNanoUsd:
              estimates[kind].automaticNanoUsd === null
                ? null
                : String(estimates[kind].automaticNanoUsd),
            overrideNanoUsd:
              estimates[kind].overrideNanoUsd === null
                ? null
                : String(estimates[kind].overrideNanoUsd),
            effectiveNanoUsd:
              estimates[kind].effectiveNanoUsd === null
                ? null
                : String(estimates[kind].effectiveNanoUsd),
  ...
  return {
    ...
    summary: {
      ...
      oversoldCredentialCount: pools.filter(
        (pool) => pool.allocatedUnits > pool.capacityUnits,
      ).length,
    },
    pools,
  };
...
export function patchCredentialQuotaEstimates(
  credentialId: string,
  ...
  setCredentialQuotaEstimates(credentialId, {
    ...(Object.hasOwn(input, "5h")
      ? { "5h": nullableQuotaOverride(input["5h"]) }
      : {}),
    ...(Object.hasOwn(input, "7d")
...
export function createSubscription(input: Record<string, unknown>) {
  ...
  if (!credential.enabled)
    throw new HttpError(
      ...
      "Disabled credential cannot receive new allocations",
    );
  ...
  if (!tenant?.enabled)
    throw new HttpError(
      400,
      "tenant_disabled",
      "Disabled tenant cannot receive new allocations",
    );
  const tenantUser = getTenantOwnerUser(tenantId);
  if (!tenantUser?.enabled)
    throw new HttpError(
      400,
      "tenant_user_not_available",
      "Tenant user must be active before receiving a subscription",
    );
  ...
  return insertTenantSubscription({
    ...
    name:
      clean(input.name) ||
      `${providerPlanLabel(credential.provider, credential.planType)} ${units}/${unitsPerCredential}`,
    units,
    unitsPerCredential,
...
export function patchSubscription(id: string, input: Record<string, unknown>) {
  ...
  const units =
    input.units === undefined
      ? current.units
      : positiveNumber(input.units, current.units);
  const defaultUnitsPerCredential =
    credentialId === current.credentialId
      ? current.unitsPerCredential
      : providerCapacityUnits(credential.provider, credential.planType);
  const unitsPerCredential =
    input.unitsPerCredential === undefined
  ...
  return updateTenantSubscription(id, {
    ...
    ...(input.startsAt !== undefined
      ? { startsAt: date(input.startsAt) || current.startsAt }
      : {}),
...
export function removeSubscription(id: string) {
  if (!deleteTenantSubscription(id))
    throw new HttpError(
      404,
      "subscription_not_found",
      "Subscription not found",
...
function nullableQuotaOverride(value: unknown) {
  ...
  try {
    ...
    throw new HttpError(
      400,
      "invalid_estimated_quota",
      "Estimated quota must be greater than zero",
    );
  }
}
