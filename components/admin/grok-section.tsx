...
export function GrokSection() {
  ...
  return <Card><CardHeader><CardTitle>Grok 凭据与订阅</CardTitle></CardHeader><CardContent className="grid gap-4">
    <div className="flex flex-wrap gap-2"><Button onClick={startOAuth}>连接 Grok 订阅</Button>{session && <><span className="self-center font-mono text-sm">验证码 {session.userCode}</span><Button variant="outline" onClick={pollOAuth}>我已授权</Button></>}</div>
    <div className="grid gap-2 md:grid-cols-[1fr_2fr_auto]"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="名称（可选）"/><Input type="password" value={key} onChange={(e) => setKey(e.target.value)} placeholder="xAI API Key"/><Button onClick={addKey} disabled={!key.trim()}>添加 API Key</Button></div>
