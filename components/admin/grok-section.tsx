...
export function GrokCredentialCards() {
  ...
  async function createChannel(credential: GrokCredentialRecord) { const catalogResponse = await fetch("/api/model-catalog?provider=grok", { cache: "no-store" }); if (!catalogResponse.ok) return toast.error(await errorText(catalogResponse)); const catalog = await catalogResponse.json() as { data?: string[] }; const modelAllowlist = Array.isArray(catalog.data) ? catalog.data.filter((model) => typeof model === "string" && model.trim()) : []; if (!modelAllowlist.length) return toast.error("Grok 上游没有返回可声明的模型"); const response = await fetch("/api/admin/channels", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "grok", name: `Grok · ${credential.email || credential.subject || credential.id}`, credentialIds: [credential.id], baseUrl: credential.authType === "oauth" ? "https://cli-chat-proxy.grok.com/v1" : "https://api.x.ai/v1", modelAllowlist }) }); if (!response.ok) return toast.error(await errorText(response)); toast.success(`Grok 通道已创建，已声明 ${modelAllowlist.length} 个模型`); }
