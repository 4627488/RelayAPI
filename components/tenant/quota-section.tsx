...
function SubscriptionCard({ subscription }: { subscription: TenantQuotaReport["subscriptions"][number] }) { return <Card><CardHeader><div className="flex items-center justify-between gap-2"><CardTitle>{subscription.name}</CardTitle><Badge variant="outline">{subscription.units}/{subscription.unitsPerCredential}</Badge></div><CardDescription>{subscription.expiresAt ? `有效至 ${new Date(subscription.expiresAt).toLocaleString("zh-CN")}` : "长期有效"}</CardDescription></CardHeader><CardContent className="flex flex-col gap-4">{(["5h", "7d"] as const).map((kind) => <QuotaLine key={kind} kind={kind} window={subscription.windows[kind]} />)}</CardContent></Card>; }
function QuotaLine({ kind, window }: { kind: "5h" | "7d"; window?: TenantQuotaReport["subscriptions"][number]["windows"]["5h"] }) { const limit = Number(window?.limitNanoUsd || 0); const used = Number(window?.settledNanoUsd || 0) + Number(window?.reservedNanoUsd || 0); const percent = limit > 0 ? Math.min(100, used / limit * 100) : 0; return <div className="flex flex-col gap-2"><div className="flex items-center justify-between gap-2"><span className="font-medium">{kind === "5h" ? "5 小时" : "7 天"}</span><span className="text-sm text-muted-foreground">{formatUsd(String(used))} / {formatUsd(String(limit))}</span></div><Progress value={percent} /><span className="text-xs text-muted-foreground">{window ? `重置 ${new Date(window.resetsAt).toLocaleString("zh-CN")}` : "等待首次使用或上游额度刷新"}</span></div>; }
