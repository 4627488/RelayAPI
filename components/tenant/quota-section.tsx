...
function QuotaLine({ kind, window }: { kind: "5h" | "7d"; window?: TenantQuotaReport["subscriptions"][number]["windows"]["5h"] }) {
  ...
  return <div className="flex flex-col gap-3">
    ...
    <span className="text-xs text-muted-foreground">{window ? `重置 ${new Date(window.resetsAt).toLocaleString("zh-CN")} · ${window.resetSource === "upstream" ? "上游周期" : "本地滚动周期"} · 已使用 ${Math.round(percent)}%` : "等待首次使用或上游额度刷新"}</span>
  </div>;
}
