...
export function RequestLogsWorkbench({ admin = false }: { admin?: boolean }) {
  ...
  const cacheRate =
    data.summary.tokens > 0
      ? data.summary.cached_tokens / data.summary.tokens
      : 0
      ...
      <StatStrip
        ...
        items={[
          ...
          },
          {
            ...
            detail: `缓存 ${(cacheRate * 100).toFixed(1)}%`,
          },
          {
            label: "负载",
            value: bytes(
      ...
      <DetailGroup title="用量与性能">
        <Facts
          items={[
            [
              "Token",
              `${compactTokens(log.total_tokens)}（输入 ${compactTokens(log.prompt_tokens)} · 输出 ${compactTokens(log.completion_tokens)}）`,
            ],
