  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <dl className="grid grid-cols-2 overflow-hidden rounded-lg border sm:grid-cols-3 xl:grid-cols-6">
        <Stat label="请求" value={compact(data.summary.requests)} hint={`${data.summary.errors} 个错误`} />
        <Stat
          label="错误率"
      ...
      <Card>
        <CardHeader className="border-b">
          ...
          <FieldGroup className="grid gap-2 pt-2 md:grid-cols-[minmax(16rem,1fr)_9rem_auto_auto]">
              ...
              <InputGroup>
                ...
                {query ? (
                  <InputGroupAddon align="inline-end">
                    <Button variant="ghost" size="icon-xs" onClick={() => setQuery("")} aria-label="清除搜索">
                      <XIcon />
                    </Button>
                  </InputGroupAddon>
                ) : null}
            ...
            <Field>
              ...
              <Select value={status} onValueChange={(value) => { if (value) { setStatus(value); setPage(1) } }}>
            ...
            <Collapsible open={filtersOpen} onOpenChange={setFiltersOpen} className="contents">
              <CollapsibleTrigger render={<Button variant="outline" />}>
                ...
                <ChevronDownIcon data-icon="inline-end" className={cn("transition-transform", filtersOpen && "rotate-180")} />
              </CollapsibleTrigger>
              ...
              <CollapsibleContent className="col-span-full pt-2">
                <FieldGroup className="grid gap-3 md:grid-cols-5">
                  <Field>
                    ...
                    <Select value={method} onValueChange={(value) => { if (value) { setMethod(value); setPage(1) } }}>
                      <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          <SelectItem value="all">全部方法</SelectItem>
                          {['POST', 'GET', 'PUT', 'PATCH', 'DELETE'].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}
                        </SelectGroup>
                      </SelectContent>
                    </Select>
                  </Field>
                  <Field>
                    ...
                    <Input id="log-model" value={model} onChange={(event) => { setModel(event.target.value); setPage(1) }} placeholder="例如 gpt-5.6" />
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="log-latency">最小耗时</FieldLabel>
                    <Input id="log-latency" value={minLatency} onChange={(event) => { setMinLatency(event.target.value); setPage(1) }} type="number" min="0" placeholder="毫秒" />
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="log-from">开始时间</FieldLabel>
                    <Input id="log-from" value={from} onChange={(event) => { setFrom(event.target.value); setPage(1) }} type="datetime-local" />
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="log-to">结束时间</FieldLabel>
                    <Input id="log-to" value={to} onChange={(event) => { setTo(event.target.value); setPage(1) }} type="datetime-local" />
                  </Field>
                </FieldGroup>
                {hasFilters ? (
                  <Button variant="ghost" size="sm" className="mt-3" onClick={resetFilters}>
                    <XIcon data-icon="inline-start" />清除全部筛选
                  </Button>
                ) : null}
              </CollapsibleContent>
            </Collapsible>
            ...
            <Table className={cn(loading && "opacity-60")}>
              ...
              <TableBody>
                {data.items.map((log) => (
                  ...
                  >
                    <TableCell className="whitespace-nowrap pl-4 text-muted-foreground">{dateTime(log.started_at)}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        {log.stream && log.status_code !== 101 ? <span className="text-xs text-muted-foreground">流式</span> : null}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    {admin ? (
                      <TableCell>
                        <p className="max-w-40 truncate text-sm">{log.tenant_name || log.tenant_id}</p>
                    ...
                    <TableCell className="whitespace-nowrap text-right tabular-nums">
                      ...
                      {log.ttft_ms != null ? <p className="text-xs text-muted-foreground">首字节 {log.ttft_ms} ms</p> : null}
                    </TableCell>
                    <TableCell className="pr-4 text-right tabular-nums">{money(log.cost_nano_usd)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <CardFooter className="justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">{data.total} 条 · 第 {data.page}/{totalPages} 页</span>
            <Select value={String(pageSize)} onValueChange={(value) => { if (value) { setPageSize(Number(value)); setPage(1) } }}>
              <SelectTrigger size="sm"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  {[25, 50, 100, 200].map((value) => <SelectItem key={value} value={String(value)}>{value} / 页</SelectItem>)}
                </SelectGroup>
              </SelectContent>
            </Select>
          </div>
          <div className="flex gap-1">
            <Button variant="outline" size="icon-sm" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} aria-label="上一页"><ChevronLeftIcon /></Button>
            <Button variant="outline" size="icon-sm" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)} aria-label="下一页"><ChevronRightIcon /></Button>
          </div>
        </CardFooter>
      </Card>
...
function Stat({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div className="border-r border-b p-3 last:border-r-0 sm:p-4 xl:border-b-0">
    ...
    <div className="flex flex-col gap-5 p-4 sm:p-5">
      {errorTitle || errorMessage ? (
        <Alert variant="destructive">
          ...
          {errorMessage ? <AlertDescription className="break-words">{errorMessage}</AlertDescription> : null}
        </Alert>
      ) : null}
        ...
        <DetailGroup title="阶段耗时">
          <div className="grid grid-cols-2 gap-x-5 gap-y-3 sm:grid-cols-3">
            {Object.entries(timings).map(([key, value]) => (
              <Detail key={key} label={timingLabel(key)} value={`${value} ms`} />
            ))}
          </div>
        </DetailGroup>
      ) : null}
              ...
              <TableBody>
                {costRows.map(([label, tokens, rate]) => (
                  <TableRow key={label}>
                    ...
                    <TableCell className="text-right tabular-nums">{tokens}</TableCell>
                    <TableCell className="text-right tabular-nums">{rate} nanoUSD</TableCell>
                    <TableCell className="text-right tabular-nums">{money(tokens * rate)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
function RequestSection({ detail }: { detail: RequestLogDetail }) {
  const headers = hasJSONObject(detail.request_headers) ? prettyJSON(detail.request_headers) : ""
  return (
    <div className="flex flex-col gap-5 p-4 sm:p-5">
      {headers ? <Payload title="Headers" value={headers} /> : null}
      {detail.request_body || detail.request_body_bytes ? (
        <Payload title="Body" value={prettyJSON(detail.request_body)} bytes={detail.request_body_bytes} truncated={detail.request_body_truncated} />
      ) : null}
    </div>
  )
}
...
function ForwardedSection({ detail }: { detail: RequestLogDetail }) {
  const headers = hasJSONObject(detail.forwarded_headers) ? prettyJSON(detail.forwarded_headers) : ""
  return (
    <div className="flex flex-col gap-5 p-4 sm:p-5">
      {headers ? <Payload title="Headers" value={headers} /> : null}
      {detail.forwarded_body ? (
        <Payload title="转换后 Body" value={prettyJSON(detail.forwarded_body)} bytes={detail.forwarded_body_bytes} truncated={detail.forwarded_body_truncated} />
      ) : detail.forwarded_body_bytes ? (
        <p className="text-sm text-muted-foreground">转发正文与客户端请求相同，未重复存储（{bytes(detail.forwarded_body_bytes)}）。</p>
      ) : null}
    </div>
  )
}
...
function Facts({ items }: { items: Array<[string, string | undefined]> }) {
  ...
  return (
    <dl className="grid gap-x-5 gap-y-3 sm:grid-cols-2">
      {visible.map(([label, value]) => <Detail key={label} label={label} value={value ?? ""} />)}
    </dl>
  )
}
...
function Payload({ title, value, bytes: byteCount, truncated = false }: { title: string; value: string; bytes?: number; truncated?: boolean }) {
  if (!value) return null
  return (
    <section className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-medium">{title}</h3>
        {byteCount != null ? <span className="text-xs text-muted-foreground">{bytes(byteCount)}</span> : null}
        {truncated ? <Badge variant="outline">已截断</Badge> : null}
        <CopyButton value={value} label={`复制 ${title}`} className="ml-auto" />
      </div>
      <pre className="max-h-[32rem] overflow-auto rounded-lg border bg-muted/40 p-3 text-xs leading-relaxed whitespace-pre-wrap break-all">{value}</pre>
    </section>
  )
}
...
function CopyButton({ value, label, className }: { value: string; label: string; className?: string }) {
  ...
  return (
    <Button variant="ghost" size="icon-xs" className={className} onClick={() => void copy()} aria-label={label}>
      <CopyIcon />
    </Button>
  )
}
...
function hasJSONObject(value?: string) {
  ...
  try {
    ...
    return typeof parsed === "object" && parsed !== null ? Object.keys(parsed).length > 0 : Boolean(parsed)
  } catch {
