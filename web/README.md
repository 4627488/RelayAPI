...
- 列表集中展示调度状态、认证方式、已发布模型、连接出口和额度观测；请求成功次数不代表剩余额度，“可调度”不代表已通过连通性测试。
- 详情使用 Sheet，按运行状态、模型发布、连接设置和凭据分区。保存只提交当前分区；其他分区草稿保留，关闭前确认放弃。
- 模型目录刷新只更新候选项，不能自动改变已发布模型；刷新失败也必须保留选择。连接设置保存不能附带模型，以免触发后端模型发现。
- API Key 的空模型列表表示继承全部允许模型，必须通过明确的“全部可用模型”选项设置；不能因取消最后一项而静默扩大权限。
- 请求头明确区分保持、整体替换和清空；服务端只返回头名称，界面不应伪造原值。OAuth 和账户测试关闭后必须忽略迟到结果，并尽可能取消请求。
