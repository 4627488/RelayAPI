...
func readUsage(raw any, result *Result) {
	...
	cacheWriteTokens := maxInt(number(usage["cache_creation_input_tokens"]), number(usage["cache_write_tokens"]))
	// Kimi reports uncached input, cache reads and cache writes as disjoint
	// counters. OpenAI-style input_tokens already includes cached tokens, so
	// only normalize when these explicit cache fields are present.
