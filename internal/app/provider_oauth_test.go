...
func TestMergeOAuthCredentialSettingsPreservesNonNetworkOptions(t *testing.T) {
	merged, err := mergeOAuthCredentialSettings(
		json.RawMessage(`{"type":"kimi","access_token":"old","proxy_url":"socks5://proxy","websockets":true,"headers":{"X-Team":"one"},"model_routes":[{"public":"kimi-k3-256k","upstream":"k3"}]}`),
		json.RawMessage(`{"type":"kimi","access_token":"new","email":"new@example.com"}`),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(routes) != 1 {
		t.Fatalf("model_routes were dropped on OAuth rebind: %#v", document)
	}
