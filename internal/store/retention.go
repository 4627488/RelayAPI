...
func (s Store) compactRequestLogs(ctx context.Context, cutoff time.Time, batch int, stats *RetentionStats) (int64, error) {
	return retentionTx(ctx, s.DB, func(tx *gorm.DB) (int64, error) {
		...
		if err := tx.Raw(`SELECT date_trunc('day', l.started_at)::date AS day, l.tenant_id, l.model,
			count(*) FILTER (WHERE l.reservation_request_id IS NULL OR l.reservation_request_id = l.id) AS requests,
			COALESCE(sum(CASE WHEN (l.reservation_request_id IS NULL OR l.reservation_request_id = l.id) AND (l.status_code >= 400 OR l.status_code = 0 OR COALESCE(l.error_code, '') <> '') THEN 1 ELSE 0 END),0) AS errors,
			...
			COALESCE(sum(l.image_input_tokens),0) AS image_input_tokens,
			COALESCE(sum(l.cached_image_input_tokens),0) AS cached_image_input_tokens,
			COALESCE(sum(l.image_output_tokens),0) AS image_output_tokens,
		...
		for index := range rollups {
			...
			if err := tx.Clauses(clause.OnConflict{
				...
				DoUpdates: clause.Assignments(map[string]any{
					...
					"image_input_tokens":            gorm.Expr("usage_daily_rollups.image_input_tokens + EXCLUDED.image_input_tokens"),
					"cached_image_input_tokens":     gorm.Expr("usage_daily_rollups.cached_image_input_tokens + EXCLUDED.cached_image_input_tokens"),
					"image_output_tokens":           gorm.Expr("usage_daily_rollups.image_output_tokens + EXCLUDED.image_output_tokens"),
