...
`web/` 是基于 Vite、React 19、Tailwind CSS v4 和 Cloudflare Kumo 的独立源码应用。
页面按 `/api` 与 `/api/admin` 资源组织，视觉合同见 [DESIGN.md](DESIGN.md)。
生产镜像会把其构建产物打包进 Go 服务，因此部署时不再需要单独的 Web/Nginx 容器。
